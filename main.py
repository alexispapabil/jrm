...
from aux import format,todatetime,elapsed,timew,prettyprint
...
if __name__ == '__main__':
    ...
    if args.i:
                ...
                if args.i == 'queue':
                    ...
                    for line in lines:
                        ...
                        if line[3] == 'R':
                            ...
                            runtime = elapsed(todatetime(date, time))
                            line[-1] = timew(int(line[-1]) - runtime)
                            line[-2] = time
                        else:
                            line[-1] = timew(int(line[-1]))
                            line[-2] = '-'
                        text += format(line)
                    prettyprint(text)
                elif args.i == 'state':
        ...
        with open(args.c) as f:
            ...
            algorithms = {'FCFS': lambda j: elapsed(j.enter), 'WFP3': lambda j: math.pow(elapsed(j.enter)/j.duration,3)*j.procs}
